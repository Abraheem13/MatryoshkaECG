...
Our results favour the low-dimensionality account on two independent
grounds: the matched baselines show parity rather than an MRL advantage at
small $d$, and the embedding's effective rank of \R{EffRank} is itself close
to \R{DimSmall}. The previous version asserted the regularisation account on
the basis of a cross-architecture comparison, which cannot distinguish the
two. We flag this
